...
$borrowLists = $db->getConnection()->prepare("SELECT borrowers.*, books.name FROM borrowers CROSS JOIN books ON borrowers.book_id = books.id WHERE borrowers.borrower_id = ?");
...
<head>
  ...
  <title>My Borrow List - Siperpus</title>
          ...
          <div class="card table-responsive shadow">
            <table class="table table-striped mb-0">
              ...
              <tbody>
                ...
                <tr>
                  <td colspan="7" class="text-center">Woops! Data isn't found!</td>
                </tr>
                <?php else : while($data = $borrowListsResult->fetch_assoc()) : ?>
                <tr>
                  <td><?=$data['id'] ?></td>
                  <td><a href="https://<?=$_SERVER['SERVER_NAME'] ?>/dashboard/books/show.php?id=<?=$data['book_id'] ?>"><?=$data['name'] ?></a></td>
                  <td><?=date('l, j F Y', strtotime($data['borrow_date'])) ?></td>
                  <td><?=date('l, j F Y', strtotime($data['return_date'])) ?></td>
                  <td>
                    <?php if($data['actual_date']) : ?>
                    <?=date('l, j F Y', strtotime($data['actual_date'])) ?>
                    <?php else : ?>
                    <span class="badge badge-light">Still Borrowed</span>
                    <?php endif; ?>
                  </td>
                  <td><?php
                  if($data['actual_date']) {
                    ...
                    echo "Returned {$lateDiff->d} day(s) ago";
                    if(strtotime(date('Y-m-d')) > strtotime($data['return_date'])) echo ",&nbsp;<span class=\"text-danger\">but you late {$lateDiff->d} day(s).</span>";
                  } else {
                    $today = new DateTime(date('Y-m-d'));
                    $return = new DateTime($data['return_date']);
                    $remaining = date_diff($return, $today);
                    
                    if(strtotime(date('Y-m-d')) < strtotime($data['return_date'])) {
                      if($remaining->d > 3) {
                        echo "<span class=\"badge badge-success\">Still</span><span> {$remaining->d} day(s) remaining</span>";
                      } else {
                        echo "<span class=\"badge badge-warning\">Only</span><span> {$remaining->d} day(s) remaining</span>";
                      }
                    } else {
                      echo "<span class=\"badge badge-danger\">You Late!</span><br /><span class=\"text-danger\">You'll be charged for {$remaining->d} day(s) late return</span>.";
                    }
                  }
                  ?></td>
                  <td><?php
                  if($data['actual_date']) { ?>
                  <button class="btn btn-success disabled" disabled="disabled">Return It!</button>
                  <?php } else {
                    $today = new DateTime(date('Y-m-d'));
                    $return = new DateTime($data['return_date']);
                    $remaining = date_diff($return, $today);
                    ...
                    if(strtotime(date('Y-m-d')) < strtotime($data['return_date'])) {
                      if($remaining->d > 3) { ?>
                      <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/dashboard/my-borrows/return.php?id=<?=$data['id'] ?>" class="btn btn-success">Return It!</a>
                      <?php } else { ?>
                      <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/dashboard/my-borrows/return.php?id=<?=$data['id'] ?>" class="btn btn-warning">Return It!</a>
                      <?php }
                    } else { ?>
                    <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/dashboard/my-borrows/return.php?id=<?=$data['id'] ?>" class="btn btn-danger">Return It!</a>
                    <div class="text-danger">And you'll be charged after late return.</div>
                    <?php }
                  }
                  ?></td>
                </tr>
