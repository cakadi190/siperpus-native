...
$bookLists = $db->getConnection()->prepare("SELECT * FROM books");
$bookLists->execute();
$bookListResult = $bookLists->get_result();
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/admin/books/create.php" class="btn btn-primary btn-sm">Add New Book</a>
          </div>
          ...
          <div class="card table-responsive shadow">
            <table class="table table-striped mb-0">
              ...
              <tbody>
                <?php if($bookListResult->num_rows === 0) : ?>
                <tr>
                  <td colspan="6" class="text-center">Woops, data not found!</td>
                </tr>
                <?php else : while($data = $bookListResult->fetch_assoc()) : ?>
                <tr>
                  <td><?=$data['id'] ?></td>
                  <td><?=$data['isbn'] ?></td>
                  <td><?=$data['name'] ?></td>
                  <td><?=$data['publisher'] ?> / <?=$data['writer'] ?></td>
                  <td><?php
                  ...
                  ?></td>
                  <td>
                    <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/admin/books/show.php?id=<?=$data['id'] ?>" class="btn btn-info">Detail</a>
                    <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/admin/books/edit.php?id=<?=$data['id'] ?>" class="btn btn-warning">Edit</a>
                    <a href="https://<?=$_SERVER['SERVER_NAME'] ?>/admin/books/delete.php?id=<?=$data['id'] ?>" onclick="confirmBooking(event, this)" class="btn btn-danger">Delete</a>
                  </td>
                </tr>
                <?php endwhile; endif; ?>
              </tbody>
